-- Drop old team-based tournament tables (order matters due to FK dependencies)
DROP TABLE IF EXISTS tournament_match_games CASCADE;
...
DROP TABLE IF EXISTS tournament_requests CASCADE;
DROP TABLE IF EXISTS tournament_participants CASCADE;
DROP TABLE IF EXISTS tournaments CASCADE;

-- Recreate tournaments with the new schema
...
CREATE POLICY "tournaments_insert_service" ON tournaments FOR INSERT WITH CHECK (true);
CREATE POLICY "tournaments_update_service" ON tournaments FOR UPDATE USING (true);
CREATE POLICY "tournaments_delete_service" ON tournaments FOR DELETE USING (true);
...
CREATE POLICY "treq_insert_service" ON tournament_requests FOR INSERT WITH CHECK (true);
CREATE POLICY "treq_update_service" ON tournament_requests FOR UPDATE USING (true);

ALTER TABLE tournament_participants ENABLE ROW LEVEL SECURITY;
CREATE POLICY "tp_select_public" ON tournament_participants FOR SELECT USING (true);
CREATE POLICY "tp_insert_service" ON tournament_participants FOR INSERT WITH CHECK (true);
CREATE POLICY "tp_delete_service" ON tournament_participants FOR DELETE USING (true);
